...
def main():
    ...
    try:
        opts, _ = getopt.getopt(sys.argv[1:], "f:d",
                                ["file=", "debug"])
...
def update_code(code):
    ...
    for i in range(len(code)):
        if code[i:i + 3] == "FOR":
            ...
            default_statement = default_statement + f" {switch_var} != {rest} and "
            ...
            line = code[i+7:]
            index = line.index('\n')
            code = code[:i] + "if "+default_statement[:-4]+ ":"+code[i+7+index:]
...
def transcode(code):
    ...
    # update_code(code) dynamically changes the string 'code'.  This means that the
    # len(code) in the for loop can be incorrect and so the whole of code may not be
    # searched in one operation.  Therefore we call update_code(code) until no changes
    # have been made - this means that we are done
    new_code = ""
    changed = True
    while (changed):
